  ...
  persist(
    (set, get) => ({
      ...
      submitAll: async () => {
        ...
        if (!dob.year || !dob.month || !dob.day) throw new Error('생년월일을 완성해 주세요.');
        if (!residence.region) throw new Error('거주 지역을 선택해 주세요.');
        if (interests.length === 0) throw new Error('관심사를 1개 이상 선택해 주세요.');

        const payload = {
          birthDate: `${dob.year}-${String(dob.month).padStart(2, '0')}-${String(dob.day).padStart(2, '0')}`,
          location: residence.region!,
          interests: interests.map((label) => UI_TO_CODE[label] ?? label),
        };

        set({ isSubmitting: true });
        try {
          await putMemberInterest(payload);
          set({ ...initial });
