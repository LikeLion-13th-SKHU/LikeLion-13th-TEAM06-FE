...
export default function FinishPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(err?.message ?? '제출 중 문제가 발생했습니다.');
    }
  };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.formArea}>
        <div className={styles.card}>
          <div>
            <b>생년월일</b> : {dob.year}-{dob.month}-{dob.day}
          </div>
          <div>
            <b>거주지</b> : {residence.region}
          </div>
          <div>
            <b>관심사</b> : {interests.join(', ')}
          </div>
        </div>
      ...
      <div className={styles.actions}>
        <button className={styles.ghost} onClick={() => nav('/onboarding/interests')}>
          수정하기
        </button>
